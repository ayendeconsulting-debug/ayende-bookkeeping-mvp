...
import {
  HstPeriod, CraReport, getCraReport, createHstPeriod, filePeriod, lockPeriod,
} from '@/app/(app)/reports/hst/actions';

interface Props { initialPeriods: HstPeriod[]; initialError?: string; }

function pad(n: number) { return String(n).padStart(2, '0'); }
function formatDate(d: string) { return new Date(d).toLocaleDateString('en-CA', { year: 'numeric', month: 'short', day: 'numeric' }); }
function periodLabel(p: HstPeriod) { return `${formatDate(p.period_start)} → ${formatDate(p.period_end)} (${p.frequency})`; }
...
function getDatesForFrequency(freq: 'monthly' | 'quarterly' | 'annual'): { start: string; end: string } {
  ...
  if (freq === 'quarterly') {
    const qStartMonth = Math.floor(m / 3) * 3; const qEndMonth = qStartMonth + 2;
    const lastDay = new Date(y, qEndMonth + 1, 0).getDate();
    return { start: `${y}-${pad(qStartMonth + 1)}-01`, end: `${y}-${pad(qEndMonth + 1)}-${pad(lastDay)}` };
  }
...
function StatusBadge({ status }: { status: HstPeriod['status'] }) {
  const cfg: Record<string, string> = {
    ...
    filed:  'bg-primary-light text-primary border-primary/30 dark:bg-primary/10 dark:text-emerald-400 dark:border-primary/30',
...
function Gst34Row({ line, label, amount, highlight, editable, editValue, onEditChange }: {
  line: string; label: string; amount: number; highlight?: boolean;
  editable?: boolean; editValue?: string; onEditChange?: (v: string) => void;
}) {
  const amtClass = highlight
    ? amount < 0 ? 'text-primary font-semibold' : amount > 0 ? 'text-amber-600 font-semibold' : 'text-foreground font-semibold'
    : 'text-foreground';
  return (
    <div className={`flex items-center justify-between px-4 py-2.5 ${highlight ? 'bg-muted rounded-lg' : ''}`}>
      ...
      {editable ? (
        <div className="w-32">
          <Input type="number" min="0" step="0.01" value={editValue} onChange={(e) => onEditChange?.(e.target.value)}
            className="text-right text-sm h-8" placeholder="0.00" />
        </div>
      ) : (
        <span className={`text-sm tabular-nums ${amtClass}`}>
          ${Math.abs(amount).toFixed(2)}
          {line === '113' && amount < 0 && <span className="text-xs font-normal ml-1 text-primary">CR</span>}
...
function CreatePeriodForm({ onCreated }: { onCreated: (p: HstPeriod) => void }) {
  ...
  function handleOpen() {
    ...
    setFreq('quarterly'); setStart(dates.start); setEnd(dates.end); setError(null); setOpen(true);
  }
  function handleFreqChange(newFreq: 'monthly' | 'quarterly' | 'annual') {
    setFreq(newFreq); const dates = getDatesForFrequency(newFreq); setStart(dates.start); setEnd(dates.end);
  }
  function handleCreate() {
    ...
    startSaving(async () => {
      ...
      if (result.data) { onCreated(result.data); toastSuccess('Period created', periodLabel(result.data)); setOpen(false); }
    });
  }
    ...
    <div className="border border-border rounded-xl p-4 flex flex-col gap-3 bg-card">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="flex flex-col gap-1">
          ...
          <select value={freq} onChange={(e) => handleFreqChange(e.target.value as typeof freq)}
            className="text-sm border border-border rounded-lg px-2 py-1.5 h-8 bg-background text-foreground outline-none focus:border-primary">
      ...
      <p className="text-xs text-muted-foreground -mt-1">Dates auto-populate based on frequency. You can adjust them manually if needed.</p>
...
export function HstReportClient({ initialPeriods, initialError }: Props) {
  ...
  function handleLoad() {
    if (!selectedId) return; setError(null);
    startLoading(async () => {
  ...
  function handlePeriodCreated(p: HstPeriod) { setPeriods((prev) => [p, ...prev]); setSelectedId(p.id); setReport(null); }
    ...
    <div className="p-4 md:p-6 max-w-screen-lg mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          {report && (
            <>
              <Button variant="outline" size="sm" onClick={downloadCsv} className="flex items-center gap-1.5"><Download className="w-3.5 h-3.5" /> CSV</Button>
              <Button variant="outline" size="sm" onClick={downloadPdf} className="flex items-center gap-1.5"><FileText className="w-3.5 h-3.5" /> PDF</Button>
            </>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 mb-6">
        <div className="flex flex-wrap items-end gap-3">
          <div className="flex flex-col gap-1 min-w-[280px]">
            ...
            <div className="relative">
              <select value={selectedId} onChange={(e) => { setSelectedId(e.target.value); setReport(null); }}
                className="w-full appearance-none text-sm border border-border rounded-lg px-3 py-2 pr-8 outline-none focus:border-primary bg-background text-foreground">
          ...
          <div className="flex flex-col gap-1 w-40">
            ...
            <Input type="number" min="0" step="0.01" value={instalments}
            ...
            <>
              ...
              {selectedPeriod.status === 'open' && (
                <Button variant="outline" size="sm" onClick={handleFile} disabled={actionPending} className="flex items-center gap-1.5">
                  {actionPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CheckCircle2 className="w-3.5 h-3.5 text-primary" />}
                  Mark as Filed
                </Button>
              )}
      ...
      {periods.length === 0 && !error && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12 text-center gap-3">
            ...
            <p className="text-xs text-muted-foreground max-w-sm">Create your first HST period to generate a CRA remittance report.</p>
          </CardContent>
        </Card>
      )}
        ...
        <div className="flex flex-col gap-5">
          ...
          <Card>
            <CardHeader className="pb-3">
              ...
              <p className="text-xs text-muted-foreground">Period: {formatDate(report.period.period_start)} to {formatDate(report.period.period_end)}</p>
            </CardHeader>
            <CardContent className="flex flex-col gap-1 pt-0">
              ...
              <Gst34Row line="111" label="Instalments Already Paid" amount={report.line_111_instalments} editable editValue={instalments}
          ...
          <Card>
            <CardHeader className="pb-3"><CardTitle className="text-base">ITC Summary</CardTitle></CardHeader>
            <CardContent className="pt-0">
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="bg-primary-light dark:bg-primary/10 rounded-lg px-4 py-3">
                  <p className="text-xs text-primary mb-1">ITC Eligible (Recoverable)</p>
                  <p className="text-sm font-semibold text-primary">${report.line_106_itc_claimed.toFixed(2)}</p>
                </div>
                <div className="bg-muted rounded-lg px-4 py-3">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead>Tax Code</TableHead><TableHead>Type</TableHead>
          ...
          {report.transactions.length === 0 && (
            <Card>
              <CardContent className="py-8 text-center text-sm text-muted-foreground">No tax transactions posted in this period yet.</CardContent>
            </Card>
          )}
