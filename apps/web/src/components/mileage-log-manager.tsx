...
export function MileageLogManager({ initialData }: MileageLogManagerProps) {
  ...
  const [totals, setTotals] = useState({
    ...
    total_deduction: initialData.total_deduction,
  });
  ...
  function handleAdd() {
    ...
    if (isNaN(distance) || distance <= 0) {
      toastError('Distance must be a positive number.');
      return;
    }
        ...
        <Card>
          <CardContent className="pt-5">
            ...
            <div className="text-2xl font-semibold text-gray-900">
              {totals.total_distance.toFixed(1)} {unit}
            </div>
            <div className="text-xs text-gray-400 mt-1">{new Date().getFullYear()} total</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-5">
            ...
            <div className="text-xs text-gray-400 mt-1">Estimated tax deduction</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-5">
            ...
            <div className="text-xs text-gray-400 mt-1">
              {unit === 'km' ? 'CRA 2025 rate applied' : 'IRS 2025 rate applied'}
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => handleFormChange('start_location', e.target.value)}
                  className="mt-1"
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => handleFormChange('end_location', e.target.value)}
                  className="mt-1"
                />
              </div>
                ...
                {logs.map((log) => (
                  <TableRow key={log.id}>
                    <TableCell className="text-gray-500 whitespace-nowrap">
                      {new Date(log.trip_date).toLocaleDateString('en-CA', {
                        month: 'short',
                        day: 'numeric',
                      })}
                    </TableCell>
                    ...
                    <TableCell className="max-w-[200px]">
                      <span className="text-sm text-gray-600 truncate block">{log.purpose}</span>
                    </TableCell>
                    <TableCell className="text-right text-sm">
                      {Number(log.distance_km).toFixed(1)} {unit}
                    </TableCell>
                    <TableCell className="text-right text-sm text-gray-500">
                      ${Number(log.rate_per_km).toFixed(2)}/{unit}
                    </TableCell>
                    <TableCell className="text-right text-sm font-medium text-primary">
                      {formatCurrency(Number(log.deduction_value))}
                    </TableCell>
                    <TableCell>
                      <AlertDialog>
