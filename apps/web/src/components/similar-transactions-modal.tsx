...
interface SimilarTx { id: string; transaction_date: string; description: string; amount: number; source_account_name?: string; }

interface SimilarTransactionsModalProps {
  similar: SimilarTx[]; keyword: string;
  suggested_account_id?: string | null; suggested_account_name?: string | null; suggested_account_code?: string | null; suggested_source_account_id?: string | null;
...
export function SimilarTransactionsModal({
  similar, keyword, suggested_account_id, suggested_account_name, suggested_account_code,
  category_id, category_name, category_color, onClose, onApplied,
  ...
  function toggleAll() { if (allSelected) setSelected(new Set()); else setSelected(new Set(similar.map((t) => t.id))); }
  function toggleOne(id: string) { const next = new Set(selected); if (next.has(id)) next.delete(id); else next.add(id); setSelected(next); }
  ...
  function handleApply() {
    ...
    start(async () => {
      if (isPersonalMode) {
        ...
        if (result.success) { toastSuccess(`${result.assigned} categorized`, `Applied "${category_name}" to ${result.assigned} transactions.`); setApplied(true); onApplied(); }
        else toastError('Categorization failed', result.error ?? 'Please try again.');
      } else {
        if (!suggested_account_id) { toastError('No account suggestion available.'); return; }
        const result = await bulkClassifyTransactions({ rawTransactionIds: Array.from(selected), accountId: suggested_account_id });
        if (result.success && result.data) { toastSuccess(`${result.data.classified} classified`, `Applied "${suggested_account_name}" to ${result.data.classified} transactions.`); setApplied(true); onApplied(); }
        else toastError('Classification failed', result.error ?? 'Please try again.');
      }
    });
  }
  ...
  function handleCreateRule() {
    startRule(async () => {
      if (isPersonalMode) {
        const result = await createPersonalRuleFromSimilar({ match_type: 'keyword', match_value: keyword, budget_category_id: category_id!, priority: 10 });
        if (result.success) { setRuleCreated(true); toastSuccess('Rule created', `Future transactions matching "${keyword}" will be auto-categorized.`); }
        else toastError('Rule creation failed', result.error ?? 'Please try again.');
      } else {
        if (!suggested_account_id) return;
        const result = await createClassificationRule({ match_type: 'keyword', match_value: keyword, account_id: suggested_account_id, priority: 10 });
        if (result.success) { setRuleCreated(true); toastSuccess('Rule created', `Future transactions matching "${keyword}" will be auto-classified.`); }
        else toastError('Rule creation failed', result.error ?? 'Please try again.');
      }
    });
  }

  const suggestionLabel = isPersonalMode ? category_name : suggested_account_code ? `${suggested_account_code} – ${suggested_account_name}` : suggested_account_name;
      ...
      <div className="bg-card border border-border rounded-2xl w-full max-w-lg shadow-2xl">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              ...
              {suggestionLabel && (
                <> · {isPersonalMode ? 'Category: ' : 'Account: '}
                  <span className="font-medium" style={isPersonalMode && category_color ? { color: category_color } : undefined}>
                    {suggestionLabel}
                  </span>
                </>
              )}
        ...
        <div className="max-h-64 overflow-y-auto">
          <div className="flex items-center gap-3 px-5 py-2.5 border-b border-border bg-muted/30">
            ...
            <span className="text-xs font-medium text-muted-foreground">{selected.size} of {similar.length} selected</span>
          </div>
          <div className="divide-y divide-border">
            {similar.map((tx) => (
              <label key={tx.id} className="flex items-center gap-3 px-5 py-2.5 cursor-pointer hover:bg-muted/30 transition-colors">
                <input type="checkbox" checked={selected.has(tx.id)} onChange={() => toggleOne(tx.id)} className="rounded border-border cursor-pointer flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-foreground truncate">{tx.description}</p>
                  <p className="text-xs text-muted-foreground">{new Date(tx.transaction_date).toLocaleDateString('en-CA', { month: 'short', day: 'numeric', year: '2-digit' })}</p>
                </div>
                <span className={`text-xs font-semibold flex-shrink-0 ${tx.amount >= 0 ? 'text-primary' : 'text-foreground'}`}>
