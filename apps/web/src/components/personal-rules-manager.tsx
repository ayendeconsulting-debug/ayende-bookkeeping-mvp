...
interface PersonalRulesManagerProps { initialRules: PersonalRule[]; categories: BudgetCategoryWithSpending[]; }
interface RuleFormData { match_type: string; match_value: string; budget_category_id: string; priority: string; }
const EMPTY_FORM: RuleFormData = { match_type: 'keyword', match_value: '', budget_category_id: '', priority: '10' };
...
export function PersonalRulesManager({ initialRules, categories }: PersonalRulesManagerProps) {
  ...
  const [rules, setRules] = useState<PersonalRule[]>([...initialRules].sort((a, b) => a.priority - b.priority));
  ...
  function openCreate() { setEditingRule(null); setForm({ ...EMPTY_FORM, priority: String((rules.length + 1) * 10) }); setError(null); setDialogOpen(true); }
  function openEdit(rule: PersonalRule) {
    setEditingRule(rule);
    setForm({ match_type: rule.match_type, match_value: rule.match_value, budget_category_id: rule.budget_category_id, priority: String(rule.priority) });
    setError(null); setDialogOpen(true);
  }

  async function handleSave() {
    if (!form.match_value || !form.budget_category_id) { setError('Match value and category are required.'); return; }
    const priority = parseInt(form.priority, 10);
    if (isNaN(priority) || priority < 1) { setError('Priority must be a positive number.'); return; }
    setSaving(true); setError(null);
    const result = editingRule
      ? await updatePersonalRule(editingRule.id, { match_value: form.match_value, budget_category_id: form.budget_category_id, priority })
      : await createPersonalRule({ match_type: form.match_type, match_value: form.match_value, budget_category_id: form.budget_category_id, priority });
    setSaving(false);
    if (!result.success) { const msg = result.error ?? 'Operation failed.'; setError(msg); toastError(editingRule ? 'Failed to update rule' : 'Failed to create rule', msg); return; }
  ...
  async function handleRunRules() {
    ...
    setRunning(false);
    if (result.success && result.data) { const { matched, skipped } = result.data as { matched: number; skipped: number }; toastSuccess('Rules applied', `${matched} categorized, ${skipped} no match.`); router.refresh(); }
    else toastError('Failed to run rules', result.error ?? 'Please try again.');
  }
    ...
    <div className="p-6 max-w-screen-lg mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <AdminOnly>
            <Button onClick={openCreate} className="flex items-center gap-2"><Plus className="w-4 h-4" />New Rule</Button>
          </AdminOnly>
        </div>
      </div>
        ...
        <CardContent className="p-0">
          {rules.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm text-muted-foreground">No rules yet. Add rules to auto-categorize personal transactions.</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
                      ...
                      <TableCell className="text-sm">
                        {cat ? (
                          <span className="flex items-center gap-2">
                            <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: cat.color ?? '#0F6E56' }} />
                            {cat.name}
                            <span className="text-xs text-muted-foreground">({cat.category_type})</span>
                          </span>
                        ) : <span className="text-muted-foreground">Unknown category</span>}
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-1">
                          <AdminOnly><Button variant="ghost" size="sm" onClick={() => openEdit(rule)} className="text-muted-foreground hover:text-foreground"><Pencil className="w-3.5 h-3.5" /></Button></AdminOnly>
                          <AdminOnly>
                            <AlertDialog>
                              <AlertDialogTrigger asChild>
                                <Button variant="ghost" size="sm" className="text-muted-foreground hover:text-destructive"><Trash2 className="w-3.5 h-3.5" /></Button>
                              </AlertDialogTrigger>
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  <AlertDialogTitle>Delete rule?</AlertDialogTitle>
                                  <AlertDialogDescription>This rule will be deleted and will no longer auto-categorize matching transactions.</AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editingRule ? 'Edit Rule' : 'New Personal Rule'}</DialogTitle></DialogHeader>
          <div className="flex flex-col gap-4 mt-2">
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                <Label>Match Type</Label>
                <select value={form.match_type} onChange={(e) => setForm((f) => ({ ...f, match_type: e.target.value }))} disabled={!!editingRule} className={selectCls}>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select value={form.budget_category_id} onChange={(e) => setForm((f) => ({ ...f, budget_category_id: e.target.value }))} className={selectCls}>
            ...
            <div className="flex justify-end gap-2 mt-2">
              ...
              <Button onClick={handleSave} disabled={saving}>{saving ? 'Saving…' : editingRule ? 'Save Changes' : 'Create Rule'}</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
