...
import { Plus, AlertCircle, DollarSign, XCircle, Pencil, MoreHorizontal, TrendingUp, TrendingDown } from 'lucide-react';
...
export function ArApManager({
  ...
  function openEdit(record: ArApRecord) {
    setEditingRecord(record);
    setFormOpen(true);
  }

  function openPay(record: ArApRecord) {
  ...
  async function handleVoid(record: ArApRecord) {
    ...
    if (result.success) {
      toastSuccess('Record voided', record.party_name);
      router.refresh();
    } else {
      toastError('Failed to void record', result.error ?? 'Please try again.');
    }
  }
  ...
  const outstanding = initialRecords
    ...
    .reduce((acc, r) => {
      ...
      if (r.type === 'receivable') acc.ar += bal;
      else acc.ap += bal;
      return acc;
    }, { ar: 0, ap: 0 });
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 py-5 border-b border-gray-200 bg-white">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-xl font-semibold text-gray-900">Accounts Payable / Receivable</h1>
            <p className="text-sm text-gray-500 mt-0.5">{totalCount} records</p>
          </div>
        ...
        {(outstanding.ar > 0 || outstanding.ap > 0) && (
          <div className="grid grid-cols-2 gap-3 mb-4 max-w-sm">
            <div className="rounded-lg bg-green-50 border border-green-100 px-3 py-2">
              <div className="text-xs text-green-600 font-medium">Outstanding AR</div>
              <div className="text-sm font-semibold text-green-700">{formatCurrency(outstanding.ar)}</div>
            </div>
            <div className="rounded-lg bg-orange-50 border border-orange-100 px-3 py-2">
              <div className="text-xs text-orange-600 font-medium">Outstanding AP</div>
              <div className="text-sm font-semibold text-orange-700">{formatCurrency(outstanding.ap)}</div>
            </div>
          </div>
        )}
      ...
      <div className={cn('flex-1 overflow-auto bg-white', isPending && 'opacity-60 pointer-events-none')}>
        {initialRecords.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-gray-500">Create a receivable or payable to track what's owed.</p>
          </div>
        ) : (
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {initialRecords.map((record) => {
                ...
                return (
                  <TableRow key={record.id} className={isOverdue ? 'bg-red-50/40' : ''}>
                    <TableCell>
                      <span className={cn(
                    ...
                    <TableCell>
                      <div className="text-sm font-medium text-gray-900">{record.party_name}</div>
                      {record.party_email && (
                        <div className="text-xs text-gray-400">{record.party_email}</div>
                      )}
                    </TableCell>
                    <TableCell className="text-sm text-gray-500 max-w-[200px] truncate">
                      {record.description ?? '—'}
                    </TableCell>
                    <TableCell className={cn('text-sm whitespace-nowrap', isOverdue && 'text-red-600 font-medium')}>
                    ...
                    <TableCell className={cn(
                      ...
                      balance > 0 ? (isOverdue ? 'text-red-600' : 'text-orange-600') : 'text-gray-400',
                    )}>
                        ...
                        <DropdownMenuContent align="end">
                          {['outstanding', 'overdue', 'partially_paid'].includes(record.status) && (
                            <>
                              ...
                              <AdminOnly>
                                <DropdownMenuItem onClick={() => openPay(record)}>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      {record.party_name} — {formatCurrency(Number(record.amount))} will be voided.
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-6 py-3 border-t border-gray-200 bg-white">
          ...
          <div className="flex gap-2">
            <Button size="sm" variant="outline" disabled={currentPage <= 1} onClick={() => updateParams({ page: String(currentPage - 1) })}>
              Previous
            </Button>
            <Button size="sm" variant="outline" disabled={currentPage >= totalPages} onClick={() => updateParams({ page: String(currentPage + 1) })}>
              Next
            </Button>
          </div>
        </div>
