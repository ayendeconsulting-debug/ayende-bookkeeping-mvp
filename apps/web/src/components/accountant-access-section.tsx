...
import { ShieldCheck, Activity, Building2, CheckCircle2, XCircle, Clock } from 'lucide-react';
...
export function AccountantAccessSection() {
  ...
  useEffect(() => {
    Promise.all([getAccessRequests(), getAccountantActivity()]).then(
      ([reqs, act]) => {
        setRequests(reqs);
    ...
    startResponding(async () => {
      ...
      if (result.success) {
        toastSuccess(
          ...
          decision === 'approved'
            ? 'The accountant now has edit access for 90 days.'
                ...
                {pendingRequests.map((req) => (
                  <div key={req.id} className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-950/20 p-4">
                      ...
                      <div className="flex items-start gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-foreground truncate">
                            {req.firm?.name ?? 'Accountant Firm'}
                          </p>
                          {req.access_note && (
                            <p className="text-xs text-muted-foreground mt-0.5">{req.access_note}</p>
                          )}
                    ...
                    <div className="flex gap-2 mt-3">
                      <Button
                        size="sm"
                        className="bg-[#0F6E56] hover:bg-[#0d5f4a] text-white"
                        disabled={responding && respondingId === req.id}
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {req.status === 'approved' && req.expires_at
                            ...
                            : formatDate(req.responded_at ?? req.requested_at)}
                        </p>
                      </div>
                    </div>
                    {statusBadge(req.status)}
            ...
            {activity.length === 0 && (
              <div className="flex flex-col items-center justify-center py-8 text-center border-2 border-dashed rounded-lg">
                ...
                <p className="text-sm text-muted-foreground">
                  No accountant activity recorded yet.
                </p>
              </div>
            )}
            {activity.length > 0 && (
