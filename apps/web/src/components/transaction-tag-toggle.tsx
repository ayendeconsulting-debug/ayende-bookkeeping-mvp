...
export function TransactionTagToggle({ transactionId, isPersonal, onToggle }: TransactionTagToggleProps) {
  ...
  return (
    <div className={cn(
      ...
      <button onClick={() => handleToggle(false)}
        className={cn('px-2.5 py-0.5 rounded-full transition-colors',
          !personal ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground')}>
        Business
      </button>
      <button onClick={() => handleToggle(true)}
        className={cn('px-2.5 py-0.5 rounded-full transition-colors',
          personal ? 'bg-purple-500 text-white shadow-sm' : 'text-muted-foreground hover:text-foreground')}>
        Personal
      </button>
    </div>
  );
