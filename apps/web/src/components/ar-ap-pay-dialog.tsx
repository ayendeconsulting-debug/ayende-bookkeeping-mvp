...
export function ArApPayDialog({
  ...
  // AR: credit revenue/receivable; AP: debit expense/payable
  ...
  function handleSubmit() {
    ...
    if (amountNum > balanceOwing + 0.01) {
      setError(`Amount exceeds balance owing of $${balanceOwing.toFixed(2)}.`); return;
    }
    if (!bankAccountId) {
      setError('Please select a bank account.'); return;
    }
    if (!contraAccountId) {
      setError(`Please select a ${isReceivable ? 'revenue' : 'expense'} account.`); return;
    }

    setError(null);
    startTransition(async () => {
      const result = await payArAp(record.id, {
        amount: amountNum,
      ...
      if (result.success) {
        toastSuccess(
          amountNum >= balanceOwing ? 'Fully paid' : 'Payment recorded',
          `$${amountNum.toFixed(2)} for ${record.party_name}`,
        );
        handleClose();
        onSuccess();
      } else {
    ...
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
      <DialogContent className="max-w-md">
        ...
        <div className="flex flex-col gap-4 mt-2">
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              ...
              <Input
                ...
                placeholder={balanceOwing.toFixed(2)}
                min="0.01"
                step="0.01"
              />
            </div>
          ...
          <div className="flex flex-col gap-1.5">
            <Label>
              {isReceivable ? 'Bank Account (received into) *' : 'Bank Account (paid from) *'}
            </Label>
            <select
              value={bankAccountId}
          ...
          <div className="flex flex-col gap-1.5">
            <Label>
              {isReceivable ? 'Revenue Account (credit) *' : 'Expense Account (debit) *'}
            </Label>
            <select
