...
import {
  ...
  createVehicle, updateVehicle, recordPayment, recordLumpSumPayment, allocateUsage,
...
function AddVehicleModal({
  onClose, onCreated, isPersonalMode,
}: {
  onClose: () => void;
  onCreated: (v: FinancedVehicle) => void;
  ...
  function handleSubmit() {
    ...
    start(async () => {
      const result = await createVehicle({
        ...
        business_use_pct: isPersonalMode ? 0 : Number(form.business_use_pct),
      });
      if (result.error) { setError(result.error); toastError('Failed to create vehicle', result.error); return; }
      toastSuccess('Vehicle added', `${form.name} has been set up.`);
            ...
            <div>
              ...
              <div>
                ...
                <input type="range" min="0" max="100" value={form.business_use_pct}
                  onChange={f('business_use_pct')} className="w-full accent-teal-500" />
...
// ── Record Payment Modal (monthly + lump sum tabs) ───────────────────────────
...
function RecordPaymentModal({
  ...
  const [isPending, start]         = useTransition();
  const [error, setError]          = useState<string | null>(null);
  const [paymentMode, setPaymentMode] = useState<'monthly' | 'lump_sum'>('monthly');
  ...
  const [lumpForm, setLumpForm] = useState({ payment_date: today, lump_sum_amount: '' });

  const f  = (k: keyof typeof form)     => (e: React.ChangeEvent<HTMLInputElement>) => setForm((p)     => ({ ...p, [k]: e.target.value }));
  const lf = (k: keyof typeof lumpForm) => (e: React.ChangeEvent<HTMLInputElement>) => setLumpForm((p) => ({ ...p, [k]: e.target.value }));

  const inputClass = 'w-full text-sm border border-border rounded-lg px-3 py-2 bg-background text-foreground outline-none focus:border-accent-teal focus:ring-1 focus:ring-accent-teal';
  const tabCls = (active: boolean) =>
    `flex-1 py-2 text-xs font-medium rounded-lg transition-colors ${active ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`;

  function handleMonthly() {
    setError(null);
    const total = Number(form.total_payment), principal = Number(form.principal_amount), interest = Number(form.interest_amount);
    ...
    start(async () => {
      const result = await recordPayment(vehicle.id, { payment_date: form.payment_date, total_payment: total, principal_amount: principal, interest_amount: interest });
  ...
  function handleLumpSum() {
    setError(null);
    const amount = Number(lumpForm.lump_sum_amount);
    if (!amount || amount <= 0) { setError('Enter a valid lump sum amount.'); return; }
    start(async () => {
      const result = await recordLumpSumPayment(vehicle.id, { payment_date: lumpForm.payment_date, lump_sum_amount: amount });
      if (result.error) { setError(result.error); toastError('Payment failed', result.error); return; }
      toastSuccess('Lump sum applied', `Remaining balance: ${fmt(result.data!.balance_after)}`);
      onRecorded();
    });
  }
      ...
      <div className="bg-card border border-border rounded-2xl w-full max-w-md shadow-2xl">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex gap-1 p-1 bg-muted rounded-xl">
            <button className={tabCls(paymentMode === 'monthly')}   onClick={() => { setPaymentMode('monthly');   setError(null); }}>Monthly Payment</button>
            <button className={tabCls(paymentMode === 'lump_sum')}  onClick={() => { setPaymentMode('lump_sum'); setError(null); }}>Lump Sum</button>
          </div>
          ...
          {paymentMode === 'monthly' ? (
            ...
            <>
              <div>
                <label className="text-xs font-medium text-foreground block mb-1">Payment Date</label>
                <input type="date" value={lumpForm.payment_date} onChange={lf('payment_date')} className={inputClass} />
              </div>
              <div>
                <label className="text-xs font-medium text-foreground block mb-1">Lump Sum Amount</label>
                <input type="number" value={lumpForm.lump_sum_amount} onChange={lf('lump_sum_amount')} placeholder="5000" className={inputClass} />
              </div>
              <div className="rounded-lg px-3 py-2 text-xs text-accent-teal" style={{ backgroundColor: 'var(--de-accent-teal-muted)' }}>
                Reduces your remaining balance and recalculates your payment schedule automatically. Posts as: DR Loan Payable · CR Owner Contribution
              </div>
            </>
          )}
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border">
          ...
          <button onClick={paymentMode === 'monthly' ? handleMonthly : handleLumpSum} disabled={isPending}
            className="px-4 py-2 text-sm font-medium text-primary-foreground bg-primary rounded-lg hover:bg-primary-hover transition-colors flex items-center gap-2 disabled:opacity-50">
            {isPending && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
            {paymentMode === 'monthly' ? 'Record Payment' : 'Apply Lump Sum'}
          </button>
        </div>
      </div>
    </div>
...
// ── Allocate Modal (Pro/Freelancer only — unchanged) ─────────────────────────

function AllocateModal({ vehicle, onClose, onAllocated }: { vehicle: FinancedVehicle; onClose: () => void; onAllocated: () => void }) {
  const [isPending, start] = useTransition();
  const [error, setError]  = useState<string | null>(null);
          ...
          {result ? (
            <div className="space-y-3">
              ...
              <div className="rounded-xl border border-border divide-y divide-border text-sm">
                <div className="flex justify-between px-4 py-2.5"><span className="text-muted-foreground">Total interest in period</span><span className="font-medium text-foreground">{fmt(result.total_interest)}</span></div>
                <div className="flex justify-between px-4 py-2.5"><span className="text-muted-foreground">Business ({result.business_pct}%) {'->'} Vehicle Expense</span><span className="font-semibold text-accent-teal">{fmt(result.business_amount)}</span></div>
                <div className="flex justify-between px-4 py-2.5"><span className="text-muted-foreground">Personal ({100 - result.business_pct}%) {'->'} Owner Draw</span><span className="font-semibold text-foreground">{fmt(result.personal_amount)}</span></div>
              </div>
            </div>
          ) : (
            <>
...
function VehicleDetail({
  vehicle: initialVehicle, onBack, isPersonalMode,
}: {
  vehicle: FinancedVehicle;
  onBack: () => void;
  isPersonalMode?: boolean;
  ...
  const statTiles = [
    ...
    ...(!isPersonalMode ? [{ label: 'Business Use', value: `${vehicle.business_use_pct}%`, icon: Percent }] : []),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card border border-border rounded-2xl p-5">
        ...
        <div className={`grid gap-3 ${statTiles.length === 3 ? 'grid-cols-3' : 'grid-cols-2 md:grid-cols-4'}`}>
          {statTiles.map((s) => (
              ...
              <div key={p.id} className="flex items-center gap-4 px-5 py-3">
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-foreground">
                    {new Date(p.payment_date).toLocaleDateString('en-CA', { year: 'numeric', month: 'short', day: 'numeric' })}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {p.interest_amount > 0
                      ? `Principal ${fmt(p.principal_amount)} · Interest ${fmt(p.interest_amount)}`
                      : `Lump sum · Principal ${fmt(p.principal_amount)}`}
                  </p>
                </div>
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden">
        <div className="px-5 py-3.5 border-b border-border flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground ml-1">(from current balance)</span>
        </div>
        {loadingSchedule ? (
...
// ── Vehicle Manager (root export) ────────────────────────────────────────────

export function VehicleManager({ initialVehicles, isPersonalMode }: { initialVehicles: FinancedVehicle[]; isPersonalMode?: boolean }) {
  ...
  if (selected) {
    return <VehicleDetail vehicle={selected} onBack={() => setSelected(null)} isPersonalMode={isPersonalMode} />;
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {vehicles.length === 0 ? 'No vehicles set up yet.' : `${vehicles.length} vehicle${vehicles.length !== 1 ? 's' : ''}`}
        </p>
      ...
      {vehicles.length === 0 && (
        <div className="rounded-2xl border-2 border-dashed border-border p-12 text-center">
          ...
          <p className="text-xs text-muted-foreground mb-4">
            Add a financed vehicle to track loan payments and interest expense.
          </p>
          <button onClick={() => setShowAdd(true)}
