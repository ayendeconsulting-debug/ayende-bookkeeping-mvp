...
interface DocumentAttachmentsProps {
  ...
  /** Pre-loaded documents — passed from parent to avoid extra fetch */
...
export function DocumentAttachments({
  ...
  async function handleFileSelect(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const urlResult = await getDocumentUploadUrl({
        rawTransactionId,
        fileName: file.name,
        fileType: ext,
      ...
      // Step 2: Upload file directly to S3 (no API server involved)
  ...
  async function handleDownload(doc: StoredDocument) {
    ...
    if (result.success && result.data) {
      ...
      toastError('Download failed', result.error ?? 'Could not get download link.');
    }
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        ...
        <AdminOnly>
          <button
            ...
            disabled={uploading}
            className="flex items-center gap-1 text-xs text-[#0F6E56] hover:text-[#0a5540] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {uploading ? (
      ...
      {documents.length === 0 && !uploading && (
        <p className="text-xs text-gray-400">No attachments yet. PDF, JPG, PNG up to 10 MB.</p>
      )}
            ...
            >
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                <button
                  ...
                  className="text-gray-400 hover:text-[#0F6E56] transition-colors p-1 rounded"
                  title="Download"
                >
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          {doc.file_name} will be permanently deleted.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
