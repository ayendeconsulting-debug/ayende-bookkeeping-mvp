...
function statusVariant(s: string): 'pending' | 'classified' | 'posted' | 'review' {
  return ({ pending: 'pending', classified: 'classified', posted: 'posted', ignored: 'review' } as any)[s] ?? 'pending';
}
    ...
    <div className="p-6 max-w-screen-xl mx-auto">
      ...
      {highAnomalies.length > 0 && (
        <div className="mb-5 flex items-start gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
          ...
          <div>
            <p className="text-sm font-medium text-red-700">{highAnomalies.length} high-severity anomaly{highAnomalies.length > 1 ? 'ies' : ''} detected</p>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="col-span-2 flex flex-col gap-4">
          ...
          <Card>
            <CardHeader className="flex-row items-center justify-between pb-3">
              <CardTitle className="flex items-center gap-2"><ShieldAlert className="w-4 h-4 text-gray-400" />AI Anomaly Detection</CardTitle>
                ...
                <div className="flex flex-col gap-3">
                  {banks.map((bank) => (
                    <div key={bank.id} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium text-gray-900 truncate">{bank.institution_name}</div>
                        <div className="text-xs text-gray-400">{bank.last_synced_at ? `Synced ${new Date(bank.last_synced_at).toLocaleDateString('en-CA', { month: 'short', day: 'numeric' })}` : 'Pending sync'}</div>
                      </div>
          ...
          <Card>
            ...
            <CardContent className="pt-0">
              <div className="flex flex-col gap-2">
                ...
                <SummaryRow label="Net Income" value={formatCurrency(netIncome)} valueClass={netIncome >= 0 ? 'text-primary font-semibold' : 'text-danger font-semibold'} bold />
              </div>
              {trialBalance && !trialBalance.is_balanced && (
                <div className="mt-3 flex items-center gap-1.5 text-xs text-danger"><AlertCircle className="w-3 h-3" />Trial balance is not balanced</div>
              )}
            </CardContent>
          </Card>
        </div>
