...
interface SubscriptionStatus {
  status: string; plan: string | null; billing_cycle: string | null;
  trial_ends_at: string | null; current_period_end: string | null; days_remaining: number | null;
}
...
export default async function BillingSuccessPage() {
  ...
  try { subscription = await apiGet<SubscriptionStatus>('/billing/subscription'); } catch { /* show generic success */ }
          ...
          <div className="bg-card border border-border rounded-2xl p-6 mb-8 text-left space-y-4">
            {trialEndsAt && (
              <div className="flex items-center gap-3">
                ...
                {daysRemaining !== null && (
                  <span className="ml-auto text-xs font-semibold text-primary bg-primary-light px-2.5 py-1 rounded-full">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-foreground">
                  Auto-continues on {planLabel ?? 'Starter'} — cancel anytime from Settings
                </p>
              </div>
            </div>
          </div>
