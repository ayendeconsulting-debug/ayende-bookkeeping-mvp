...
export class LeadEnrichmentService {
  ...
  private async extractIntent(lead: Lead, apollo: ApolloEnrichment): Promise<IntentResult> {
    ...
    const messageBody = lead.notes ?? '(no message)';
...
Return ONLY this JSON:
{
  ...
  private async sendEmailAlert(lead: Lead, result: ScoreResult): Promise<void> {
    ...
    const subject = `[${result.score}/10] ${emoji} ${lead.first_name} ${lead.last_name} - ${lead.enriched_company ?? lead.company ?? 'Unknown'}`;
  ...
  <div style="background:${greenBrand};color:white;padding:20px;border-radius:12px 12px 0 0;">
    <div style="display:inline-block;background:${badgeColor};color:white;padding:6px 14px;border-radius:999px;font-weight:700;font-size:14px;margin-bottom:8px;">${emoji} ${result.score}/10</div>
  ...
  <div style="background:white;padding:20px;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 12px 12px;">
    ...
    <table style="width:100%;border-collapse:collapse;margin:16px 0;font-size:13px;">
      ...
      <tr><td style="padding:6px 0;color:#718096;">Title</td><td style="padding:6px 0;text-align:right;">${lead.enriched_title ?? '-'}</td></tr>
